...
const AssignEmproletable = () => {
  ...
  const columns = [
    { field: "asset", label: "Role Name" },
    { field: "requestId", label: "Custodian id " },
    { field: "requestorName", label: "Status" },
    { field: "location", label: "Actions" },
  ];

  const renderCustomCell = (field, row) => {
    // ✅ Map Status column (even though field is requestorName)
    ...
    // ✅ Map Actions column (even though field is location)
    if (field === "location") {
      return (
...
export default AssignEmproletable;